...
let remove a l =
	let rec aux s = function 
	| [] -> rev s
	| h::t -> if(h == a) then (rev s)@t else aux (h::s) t
in aux [] l;;

let remove_all a l =
	let rec aux s = function 
	| [] -> rev s
	| h::t -> if(h == a) then aux s t else aux (h::s) t
in aux [] l;;

let rec ldif s1 = function
	| [] -> s1
	| h::t -> ldif (remove_all h s1) t;;
...
let divide l = 
	let rec aux n m = function
		| [] -> (rev n, rev m)
		| h::[] -> (rev(h::n), rev m)
		| h::t -> match t with
			| [] -> (rev n, rev m)
			| hh::[] -> aux (h::n) (hh::m) []
			| hh::tt -> aux (h::n) (hh::m) tt
	in aux [] [] l;;